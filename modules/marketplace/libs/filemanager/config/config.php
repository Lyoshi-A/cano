...
$mpCustomDir = '';
if ($mpSeller = WkMpSeller::getSellerDetailByCustomerId(Context::getContext()->customer->id)) {
    if ($idMpSeller = $mpSeller['id_seller']) {
        $mpSellerDirPath = _PS_MODULE_DIR_ . 'marketplace/libs/source/' . $idMpSeller;

        // Create directory
        if (!file_exists($mpSellerDirPath) || !is_dir($mpSellerDirPath)) {
            // Create directory with seller Id
            @mkdir($mpSellerDirPath, 0777, true);
            // Copy index file in this directory
            copy(_PS_MODULE_DIR_ . 'marketplace/index.php', $mpSellerDirPath . '/index.php');
        }

        // If directory created or already exist then open that folder directly
        if (file_exists($mpSellerDirPath) && is_dir($mpSellerDirPath)) {
            $mpCustomDir = $idMpSeller . '/';
        }
    }
}
...
$config = [
    ...
    */
    // 'upload_dir' => '/source/',
    'upload_dir' => _MODULE_DIR_ . 'marketplace/libs/source/' . $mpCustomDir,
    /*
    |--------------------------------------------------------------------------
    ...
    */
    // 'current_path' => '../source/',
    'current_path' => '../source/' . $mpCustomDir,
    ...
    |
    */
    'thumbs_base_path' => '../source/' . $mpCustomDir,
    ...
    */
    ...
    // **********************
    'ext_img' => ['jpg', 'jpeg', 'png', 'gif', 'bmp', 'svg'], // Images
    'ext_video' => ['mov', 'mpeg', 'mp4', 'avi', 'mpg', 'wma', 'flv', 'webm'],
...
// Deleted extension ext_file, ext_music, ext_misc
return array_merge(
    $config,
    [
        'ext' => array_merge(
