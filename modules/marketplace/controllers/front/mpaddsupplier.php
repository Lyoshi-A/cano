...
class MarketplaceMpAddSupplierModuleFrontController extends ModuleFrontController
{
                    ...
                    foreach (Language::getLanguages() as $language) {
                        ...
                        if (Tools::getValue('description_' . $language['id_lang'])) {
                            if (!Validate::isCleanHtml(
                                ...
                                $this->errors[] = sprintf(
                                    $this->module->l('Product description field %s is invalid', 'mpaddsupplier'),
                                    $languageName
                                );
                            }
                        }
                        if (Tools::getValue('meta_title_' . $language['id_lang'])) {
                            if (!Validate::isGenericName(Tools::getValue('meta_title_' . $language['id_lang']))) {
                                $this->errors[] = sprintf(
                                    $this->module->l('Meta title field %s is invalid', 'mpaddsupplier'),
                                    $languageName
                                );
                            } elseif (Tools::strlen(Tools::getValue('meta_title_' . $language['id_lang'])) > 128) {
                                $this->errors[] = sprintf(
                                    $this->module->l('Meta title field is too long (%2$d chars max).', 'mpaddsupplier'),
                                    128
                                );
                            }
                        }
                        if (Tools::getValue('meta_desc_' . $language['id_lang'])) {
                            if (!Validate::isGenericName(Tools::getValue('meta_desc_' . $language['id_lang']))) {
                                $this->errors[] = sprintf(
                                    $this->module->l('Meta description field %s is invalid', 'mpaddsupplier'),
                                    $languageName
                                );
                            } elseif (Tools::strlen(Tools::getValue('meta_desc_' . $language['id_lang'])) > 255) {
                                $this->errors[] = sprintf(
                                    $this->module->l('Meta description field is too long (%2$d chars max).', 'mpaddsupplier'),
                        ...
                        if (Tools::getValue('meta_key_' . $language['id_lang'])) {
                            if (!Validate::isGenericName(Tools::getValue('meta_key_' . $language['id_lang']))) {
                                $this->errors[] = sprintf(
                                    $this->module->l('Meta key field %s is invalid', 'mpaddsupplier'),
                                    $languageName
                                );
                            }
                        }
    ...
    public function addCustomJSVars()
    {
        $jsVars = [
            ...
            'inv_language_title' => $this->l('Invalid meta title', 'mpaddsupplier'),
            'inv_language_desc' => $this->l('Invalid meta description', 'mpaddsupplier'),
            'invalid_logo' => $this->l('Invalid image extensions, only jpg, jpeg and png are allowed.', 'mpaddsupplier'),
            'static_token' => Tools::getToken(false),
            'allowed_file_size' => Configuration::get('PS_ATTACHMENT_MAXIMUM_SIZE'),
            'allowed_file_size_error' => sprintf(
                $this->l('Uploaded file size must be less than %s MB.', 'mpaddsupplier'),
    ...
    public function getBreadcrumbLinks()
    {
        ...
        $breadcrumb['links'][] = [
            'title' => $this->module->l('Suppliers', 'mpaddsupplier'),
