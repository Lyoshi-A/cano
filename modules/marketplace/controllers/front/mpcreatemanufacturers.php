...
class MarketplaceMpCreateManufacturersModuleFrontController extends ModuleFrontController
{
            ...
            if ($mpSeller && $mpSeller['active']) {
                ...
                if (Tools::getValue('id')) { // edit page
                    ...
                    if ($mpSellerId == $objMpManuf->id_seller) {
                        if ($manufacInfo = WkMpManufacturers::getMpManufacturerAllDetails($manufId)) {
                            ...
                            if (Tools::getValue('delete_logo')) {
                                // Delete from MP
                                if (file_exists(_PS_MODULE_DIR_ . 'marketplace/views/img/mpmanufacturers/' . $manufId . '.jpg')) {
                                    unlink(_PS_MODULE_DIR_ . 'marketplace/views/img/mpmanufacturers/' . $manufId . '.jpg');
                                }
                                // Delete from PS
                                if (file_exists(_PS_IMG_DIR_ . 'm/' . $manufacInfo['id_ps_manuf'] . '.jpg')) {
                                    unlink(_PS_IMG_DIR_ . 'm/' . $manufacInfo['id_ps_manuf'] . '.jpg');
                                }
                                ...
                                Tools::redirect(
                                    $this->context->link->getModuleLink(
                                        ...
                                        ['id' => $manufId, 'logo_delete_success' => 1]
                                    )
                                );
                            }
                            if ($objMpManuf->getManufacturerShopId($manufacInfo['id_ps_manuf']) != Context::getContext()->shop->id) {
                                // Other shop product edit is not allowed
                                Tools::redirect(
                                    $this->context->link->getModuleLink('marketplace', 'mpmanufacturerlist')
                                );
                            }
                            ...
                            if (file_exists($imageexist)) {
                                $this->context->smarty->assign([
                                    ...
                                    'wk_delete_logo_path' => $this->context->link->getModuleLink('marketplace', 'mpcreatemanufacturers', ['id' => $manufId, 'delete_logo' => 1]),
                        ...
                        $this->context->smarty->assign([
                            ...
                            'mp_error_message' => $this->module->l('Brand not found. Something went wrong.', 'mpcreatemanufacturers'),
                        ]);
                        $this->setTemplate(
                    ...
                    if (!Configuration::get('WK_MP_PRODUCT_MANUFACTURER_APPROVED')) { // Brand auto-approve
                        $brandStatus = 1;
                    }
                }
    ...
    public function postProcess()
    {
        if (Tools::isSubmit('submit_manufacturer') || Tools::isSubmit('submitStay_manufacturer')) {
            ...
            foreach (Language::getLanguages() as $language) {
                ...
                if (Tools::getValue('short_description_' . $language['id_lang'])) {
                    ...
                    if (!Validate::isCleanHtml($shortDesc)) {
                        $this->errors[] = sprintf(
                            $this->module->l('Short description field %s is invalid', 'mpcreatemanufacturers'),
                            $languageName
                        );
                    } elseif (Tools::strlen(strip_tags($shortDesc)) > $limit) {
                        $this->errors[] = sprintf(
                            $this->module->l('Short description field %s is too long: (%d chars max).', 'mpcreatemanufacturers'),
                            $languageName,
                            $limit
                        );
                    }
                ...
                if (Tools::getValue('description_' . $language['id_lang'])) {
                    if (!Validate::isCleanHtml(
                        ...
                        $this->errors[] = sprintf(
                            $this->module->l('Product description field %s is invalid', 'mpcreatemanufacturers'),
                            $languageName
                        );
                    }
                }
                if (Tools::getValue('meta_title_' . $language['id_lang'])) {
                    if (!Validate::isGenericName(Tools::getValue('meta_title_' . $language['id_lang']))) {
                        $this->errors[] = sprintf(
                            $this->module->l('Meta title field %s is invalid', 'mpcreatemanufacturers'),
                            $languageName
                        );
                    } elseif (Tools::strlen(Tools::getValue('meta_title_' . $language['id_lang'])) > 128) {
                        $this->errors[] = sprintf(
                            $this->module->l('Meta title field is too long (%2$d chars max).', 'mpcreatemanufacturers'),
                ...
                if (Tools::getValue('meta_desc_' . $language['id_lang'])) {
                    if (!Validate::isGenericName(Tools::getValue('meta_desc_' . $language['id_lang']))) {
                        $this->errors[] = sprintf(
                            $this->module->l('Meta description field %s is invalid', 'mpcreatemanufacturers'),
                            $languageName
                        );
                    } elseif (Tools::strlen(Tools::getValue('meta_desc_' . $language['id_lang'])) > 255) {
                        $this->errors[] = sprintf(
                            $this->module->l('Meta description field is too long (%2$d chars max).', 'mpcreatemanufacturers'),
                            call_user_func([$className, 'displayFieldName'], $className),
                            255
                        );
                    }
                ...
                if (Tools::getValue('meta_key_' . $language['id_lang'])) {
                    if (!Validate::isGenericName(Tools::getValue('meta_key_' . $language['id_lang']))) {
                        $this->errors[] = sprintf(
                            $this->module->l('Meta key field %s is invalid', 'mpcreatemanufacturers'),
                            $languageName
                        );
                    }
                }
            ...
            } elseif (!Validate::isAddress(trim($address))) {
                $this->errors[] = $this->module->l('Address is invalid.', 'mpcreatemanufacturers');
            }
            if (!trim($city)) {
                $this->errors[] = $this->module->l('City is required field.', 'mpcreatemanufacturers');
            } elseif (!Validate::isCityName(trim($city))) {
                $this->errors[] = $this->module->l('City is invalid.', 'mpcreatemanufacturers');
            }
            if (!$country) {
            ...
            if (!count($this->errors)) {
                ...
                if ($updatemanuf) {
                    $successParams = ['updatemanuf' => 1];
                } else {
                    $successParams = ['createmanuf' => 1];
                }
    ...
    public function getBreadcrumbLinks()
    {
        ...
        $breadcrumb['links'][] = [
            'title' => $this->module->l('Brands', 'mpcreatemanufacturers'),
            'url' => $this->context->link->getModuleLink('marketplace', 'mpmanufacturerlist'),
        ];
    ...
    public function setMedia()
    {
        ...
        $jsDef = [
            ...
            'allowed_file_size_error' => sprintf(
                $this->l('Uploaded file size must be less than %s MB.', 'mpcreatemanufacturers'),
