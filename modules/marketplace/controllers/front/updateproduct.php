...
class MarketplaceUpdateProductModuleFrontController extends ModuleFrontController
{
        ...
        if ($this->context->customer->isLogged()) {
            ...
            if ($seller && $seller['active']) {
                ...
                if ($mpSellerProduct->id_seller == $idSeller) {
                    ...
                    Hook::exec('actionBeforeShowUpdatedProduct', ['mp_product_details' => $mpProduct]);
            ...
            if ($seller && $seller['active']) {
                ...
                if ($mpSellerProduct->id_seller == $seller['id_seller']) {
                    ...
                    if (!Tools::getValue('product_name_' . $defaultLang)) {
                        ...
                        $hasAttribute = $objProduct->hasAttributes();
                        if ($hasAttribute
                        && (Tools::getValue('product_type') == '2' || Tools::getValue('product_type') == '3')) {
                            // Pack product or virtual product is not allowed for combination product
                            $this->errors[] = $this->module->l('You are not allowed to change combination product into pack product or virtual product.', 'updateproduct');
                        }
                        ...
                        if (empty($this->errors)) {
                            ...
                            foreach (Language::getLanguages(false) as $language) {
                                ...
                                || Configuration::get('WK_MP_SELLER_SHIPPING'))
                                && $permissionData['shippingPermission']['edit']) {
                                    if (Configuration::get('WK_MP_PRODUCT_DELIVERY_TIME')) {
                            ...
                            if (Configuration::get('WK_MP_PACK_PRODUCTS') && $productType == 2) {
                                if (Configuration::get('WK_MP_VIRTUAL_PRODUCT')) {
                                    ...
                                    if ($idPsProduct) {
                                        ...
                                        $objMpVirtualProduct->deleteProdDownloadByIdProductDownload($idProductDownload);
                                    // row delete from product download table
                                    } else {
                            ...
                            if ($productUpdated) {
                                ...
                                if (isset($deactivateAfterUpdate) && $deactivateAfterUpdate) {
                                    $successParams = ['edited_withdeactive' => 1];
                                } else {
                                    $successParams = ['edited_conf' => 1];
                                }
    ...
    public function defineJSVars($mpIdProduct, $defaultIdCategory)
    {
        $jsVars = [
            ...
            'confirm_delete_customization' => $this->module->l('Are you sure you want to delete this customization field?', 'updateproduct'),
    ...
    public function getBreadcrumbLinks()
    {
        ...
        $breadcrumb['links'][] = [
            'title' => $this->module->l('Products', 'updateproduct'),
