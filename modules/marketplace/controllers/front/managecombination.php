...
class MarketplaceManageCombinationModuleFrontController extends ModuleFrontController
{
                ...
                if ($seller && $seller['active']) {
                    if ($seller['id_seller'] == $mpProduct['id_seller']) {
                        ...
                        if ($udpatedIdPsProductAttribute) {
                            ...
                            if ($idPsProductAttribute) {
                                $combiAction = 2; // edit
                                $wkParams = ['id_combination' => $udpatedIdPsProductAttribute, 'update' => 1];
                            } else {
                                $combiAction = 1; // add
                                $wkParams = ['id_combination' => $udpatedIdPsProductAttribute, 'add' => 1];
                            }
    ...
    public function displayAjaxGetAttributeValue()
    {
        ...
        $jsondata = json_encode($attributeVal);
        echo $jsondata;
    ...
    public function getBreadcrumbLinks()
    {
        $breadcrumb = [];
        $breadcrumb['links'][] = [
            'title' => $this->module->l('Marketplace', 'managecombination'),
            'url' => $this->context->link->getModuleLink('marketplace', 'dashboard'),
        ];
        $breadcrumb['links'][] = [
            'title' => $this->module->l('Products', 'managecombination'),
