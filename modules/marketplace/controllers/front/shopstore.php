            ...
            if ($mpSeller['active']) {
                ...
                if ($mpSellerProduct) {
                    if (Module::isEnabled('mpsellerpricecomparision')) {
                        foreach ($mpSellerProduct as $productDetails) {
                            ...
                            if (isset($productDetails['is_global'], $productDetails['id_product_global'])) {
                ...
                if ($mpShopProduct) {
                    ...
                    foreach ($mpShopProduct as &$shopProduct) {
                        ...
                        $shopProduct['out_of_stock'] = (StockAvailable::outOfStock((int) $shopProduct['id_ps_product']) != 2)
                        ? StockAvailable::outOfStock((int) $shopProduct['id_ps_product'])
                        ...
                        if ($displayPriceTaxIncl) {
                            $shopProduct['retail_price'] = WkMpHelper::displayPrice($product->getPriceWithoutReduct());
                            $shopProduct['price'] = WkMpHelper::displayPrice($product->getPrice(true));
                        } else {
                            $shopProduct['retail_price'] = WkMpHelper::displayPrice($product->getPriceWithoutReduct(true));
                            $shopProduct['price'] = WkMpHelper::displayPrice($product->getPrice(false));
                        }
                        ...
                        if ($productHasCombination) {
                            ...
                            if ($defaultAttributeId) {
                                ...
                                if ($displayPriceTaxIncl) {
                                    $shopProduct['retail_price'] = WkMpHelper::displayPrice($product->getPriceWithoutReduct(false, $defaultAttributeId));
                                    $shopProduct['price'] = WkMpHelper::displayPrice($product->getPrice(true, $defaultAttributeId));
                                } else {
                                    $shopProduct['retail_price'] = WkMpHelper::displayPrice($product->getPriceWithoutReduct(true, $defaultAttributeId));
                                    $shopProduct['price'] = WkMpHelper::displayPrice($product->getPrice(false, $defaultAttributeId));
                                }
                    ...
                    if ($idCategory = Tools::getValue('id_category')) {
                        ...
                        Media::addJSDef([
                            'requestSortProducts' => $this->context->link->getModuleLink(
                                'marketplace',
                                'shopstore',
                                ['mp_shop_name' => $shopLinkRewrite, 'id_category' => (int) $idCategory]
                            ),
                        ]);
                        ...
                        Media::addJSDef([
                            'requestSortProducts' => $this->context->link->getModuleLink(
                                'marketplace',
                                'shopstore',
                                ['mp_shop_name' => $shopLinkRewrite, 'p' => (int) $page]
                            ),
                        ]);
                    } else {
                        Media::addJSDef([
                ...
                $this->context->smarty->assign([
                    ...
                    'myAccount' => 'index.php?controller=authentication&back=' . urlencode($this->context->link->getModuleLink('marketplace', 'shopstore', ['mp_shop_name' => $shopLinkRewrite])),
                ]);
            } else {
