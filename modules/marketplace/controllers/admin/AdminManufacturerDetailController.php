...
class AdminManufacturerDetailController extends ModuleAdminController
{
    public function __construct()
    {
        ...
        parent::__construct();
        $this->toolbar_title = $this->l('Brands');
        ...
        if ($this->display == 'add') {
            ...
            if (Tools::getValue('id_wk_mp_manufacturers')) {
                ...
                if (!in_array($objMpSeller->id_shop_group, Shop::getContextListShopID())) {
                    // For shop group
                    $this->errors[] = $this->l('You can not add or edit a brands in this shop context: select a shop instead of a group of shops.');
                ...
                if (Tools::getValue('delete_logo')) {
                    ...
                    Tools::redirectAdmin(
                        $this->context->link->getAdminLink('AdminManufacturerDetail') . '&updatewk_mp_manufacturers=&id_wk_mp_manufacturers=' . (int) $manufId . '&conf=7'
                    );
                }
                ...
                if ($imageexist) {
                    $this->context->smarty->assign([
                        'wk_delete_logo_path' => $this->context->link->getAdminLink('AdminManufacturerDetail') . '&updatewk_mp_manufacturers=&id_wk_mp_manufacturers=' . (int) $manufId . '&delete_logo=1',
    ...
    public function postProcess()
    {
        ...
        if (Tools::isSubmit('submit_manufacturer') || Tools::isSubmit('submitStay_manufacturer')) {
            if (Tools::getValue('id_wk_mp_manufacturers')) {
                ...
                if ($mpSeller = WkMpSeller::getSellerByCustomerId($idCustomer)) {
            ...
            foreach (Language::getLanguages() as $language) {
                ...
                if (Tools::getValue('short_description_' . $language['id_lang'])) {
                    ...
                    if (!Validate::isCleanHtml($shortDesc)) {
                        $this->errors[] = sprintf(
                            $this->l('Short description field %s is invalid'),
                            $languageName
                        );
                    } elseif (Tools::strlen(strip_tags($shortDesc)) > $limit) {
                        $this->errors[] = sprintf(
                            $this->l('Short description field %s is too long: (%d chars max).'),
                            $languageName,
                            $limit
                        );
                    }
                ...
                if (Tools::getValue('description_' . $language['id_lang'])) {
                    if (!Validate::isCleanHtml(
                        ...
                        $this->errors[] = sprintf($this->l('Product description field %s is invalid'), $languageName);
                    }
                }
                ...
                if (Tools::getValue('meta_desc_' . $language['id_lang'])) {
                    if (!Validate::isGenericName(Tools::getValue('meta_desc_' . $language['id_lang']))) {
                        ...
                        $this->errors[] = sprintf(
                            ...
                            call_user_func([$className, 'displayFieldName'], $className), 255
                        );
                    }
                }
            ...
            if (!trim($city)) {
                ...
                $this->errors[] = $this->l('City is invalid.');
            }
            if (!$country) {
    ...
    public function ajaxProcessFindSellerManufacturers()
    {
        ...
        if ($mpSeller && $mpSeller['active']) {
            ...
            if ($manufacturers) {
                exit(json_encode($manufacturers)); // close ajax
            }
        }
