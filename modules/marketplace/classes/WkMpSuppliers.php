...
class WkMpSuppliers extends ObjectModel
{
    ...
    public static $definition = [
        ...
        'fields' => [
            ...
            'id_ps_supplier' => ['type' => self::TYPE_INT],
            'id_ps_supplier_address' => ['type' => self::TYPE_INT],
        ],
    ];
    ...
    public static function deleteSupplier($idSupplier)
    {
        if ($idSupplier) {
            ...
            if ($idPsSupplier) {
                $current_logo_file = _PS_IMG_DIR_ . 'su/' . $idSupplier . '.jpg';
    ...
    {
        $sellerSuppliers = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS(
            'SELECT * FROM `' . _DB_PREFIX_ . 'wk_mp_suppliers`
    ...
    public function getSupplierShopId($psIdSupplier)
    {
        return Db::getInstance(_PS_USE_SQL_SLAVE_)->getValue(
            'SELECT `id_shop` FROM `' . _DB_PREFIX_ . 'supplier_shop`
            WHERE `id_supplier` = ' . (int) $psIdSupplier
        );
    }
    ...
    public function uploadSupplierLogoToPs($psIdSupplier, $mpIdSupplier, $mpImageDir)
    {
        ...
        if (file_exists($oldPath)) {
            $newPath = _PS_IMG_DIR_ . 'su/' . $psIdSupplier;
    ...
    public static function sendSupplierMailToSeller($mpSupplierId, $active)
    {
        $objMpSupplier = new WkMpSuppliers();
        if ($supplierInfo = $objMpSupplier->getMpSupplierAllDetails($mpSupplierId)) {
            $status = $active; // go for Active supplier

            if ($supplierInfo['id_ps_supplier']) { // Update in PS
                WkMpSuppliers::changeStatus($status, $supplierInfo['id_ps_supplier']);
            }

            $objSeller = new WkMpSeller($supplierInfo['id_seller']);
            $idLang = $objSeller->default_lang;
            ...
            if ($status) {
                ...
                Mail::Send(
                    $idLang,
                    'supplier_deactive',
                    Mail::l('Supplier deactive', $idLang),
                    $templateVars,
                    $businessEmail,
                    null,
                    null,
