...
class WkMpProductFeature
{
    ...
    public static function addFeaturesToPS($idPsProduct, $idFeature, $idValue, $customVal = 0)
    {
        ...
        SpecificPriceRule::applyAllRules([(int) $idPsProduct]);
        if ($idValue) {
            return $idValue;
        }
    }
    ...
    public static function checkCustomFeatureValue($params, $rules, $i, $objMp, $className)
    {
        foreach (Language::getLanguages(false) as $language) {
            ...
            if (Tools::strlen($customValue) > $rules['sizeLang']['value']) {
                ...
                exit(json_encode($data));
            } elseif (!call_user_func(['Validate', $rules['validateLang']['value']], $customValue)) {
                $data = [
                    'status' => 'ko',
                    'tab' => 'wk-feature',
                    'multilang' => '0',
     ...
     *
     * [type] TRUE  =>  if feature is used for any product , FALSE =>  if not used
     *
     * @return bool
     */
