...
class WkMpManufacturers extends ObjectModel
{
    ...
    public static $definition = [
        ...
        'fields' => [
            'id_seller' => ['type' => self::TYPE_INT],
            'id_ps_manuf' => ['type' => self::TYPE_INT],
            'id_ps_manuf_address' => ['type' => self::TYPE_INT],
        ],
    ];
    ...
    public function delete()
    {
        ...
        if ($idPsManuf) {
            ...
            Db::getInstance()->update('product', ['id_manufacturer' => 0], 'id_manufacturer =' . (int) $idPsManuf);
        }

        Db::getInstance()->delete('wk_mp_manufacturers', 'id_wk_mp_manufacturers =' . (int) $mpManufID);

        // unlink logo from marketplace directory
        if (file_exists(_PS_MODULE_DIR_ . 'marketplace/views/img/mpmanufacturers/' . $mpManufID . '.jpg')) {
            unlink(_PS_MODULE_DIR_ . 'marketplace/views/img/mpmanufacturers/' . $mpManufID . '.jpg');
        }
    ...
    {
        $sellerManufacturers = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS(
            'SELECT * FROM `' . _DB_PREFIX_ . 'wk_mp_manufacturers`
        ...
        if (empty($sellerManufacturers)) {
            ...
            foreach ($sellerManufacturers as &$manufacturers) {
                if ($manufacturers['id_ps_manuf']) {
                    $objManufacturer = new Manufacturer($manufacturers['id_ps_manuf']);
                    $manufacturers['name'] = $objManufacturer->name;
                    $manufacturers['active'] = $objManufacturer->active;
                    $manufacturers['id_shop'] = $this->getManufacturerShopId($manufacturers['id_ps_manuf']);
                }
            }
        }
    ...
    public function getManufacturerShopId($psIdManuf)
    {
        return Db::getInstance(_PS_USE_SQL_SLAVE_)->getValue(
            'SELECT `id_shop` FROM `' . _DB_PREFIX_ . 'manufacturer_shop`
            WHERE `id_manufacturer` = ' . (int) $psIdManuf
        );
    }
