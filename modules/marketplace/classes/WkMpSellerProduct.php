...
class WkMpSellerProduct extends ObjectModel
{
    ...
    public static $definition = [
        ...
        'fields' => [
            'id_seller' => ['type' => self::TYPE_INT, 'required' => false],
            'id_ps_product' => ['type' => self::TYPE_INT, 'required' => false],
            'id_mp_shop_default' => ['type' => self::TYPE_INT, 'required' => false],
            'id_mp_duplicate_product_parent' => ['type' => self::TYPE_INT, 'required' => false],
            'status_before_deactivate' => ['type' => self::TYPE_BOOL, 'validate' => 'isBool'],
            'admin_assigned' => ['type' => self::TYPE_BOOL, 'validate' => 'isBool'],
            'admin_approved' => ['type' => self::TYPE_BOOL, 'validate' => 'isBool'],
            'is_pack_product' => ['type' => self::TYPE_BOOL, 'validate' => 'isBool'],
            'pack_stock_type' => ['type' => self::TYPE_INT],
    ...
    public function deleteSellerProduct($mpIdProduct)
    {
        ...
        Hook::exec('actionMpProductDelete', ['id_mp_product' => (int) $mpIdProduct]);
        if ($objMpProduct->id) {
    ...
    public function saveSellerProductToPs($productInfo, $active = false, $editIdPsProduct = false)
    {
        ...
        foreach (Language::getLanguages(false) as $lang) {
            ...
            if (isset($productInfo['short_description'][$lang['id_lang']])) {
                $wkSummarry = $productInfo['short_description'][$lang['id_lang']];
                $product->description_short[$lang['id_lang']] = $wkSummarry;
            }
        ...
        if ($psIdProduct = $product->id) {
            if (Configuration::get('PS_SEARCH_INDEXATION')) {
                Search::indexation(false, $psIdProduct);
            }
            ...
            && isset($productInfo['product_feature'], $productInfo['default_lang'])
    ...
    public static function getSellerProduct(
        ...
        Hook::exec(
            ...
            ['seller_product_list' => &$mpProducts]
        );
        ...
        if (!empty($mpProducts)) {
            ...
            foreach ($mpProducts as &$mpProduct) {
                ...
                $mpProduct['price_per_context_with_sign'] = WkMpHelper::displayPrice($mpProduct['price_per_context']);
            }
    ...
    public static function sendMail(
        ...
        if ($idPsProductAttribute) {
            ...
            if (_PS_VERSION_ >= '8.0.0') {
                $attributes = ProductAttribute::getAttributes($idLang, true);
            } else {
        ...
        $templateVars = [
            ...
            '{product_price}' => WkMpHelper::displayPrice($productPrice),
            '{quantity}' => $quantity,
    ...
    {
        Hook::exec('actionBeforeDuplicateMPProduct', ['id_mp_product' => $originalMpProductId]);
            ...
            if ($duplicatePsProductId = $this->copyMpProductToPs($originalMpProductId)) {
                ...
                if ($objMpProduct->save()) {
                    ...
                    if (Configuration::get('WK_MP_SELLER_SHIPPING')) {
                        ...
                        $objMpShipping->assignShippingOnProduct($originalMpProductId, 0);
                    }

                    Hook::exec('actionAfterAddMPProduct', ['id_mp_product' => $originalMpProductId]);
    ...
    public static function getMpProductTaxIncludedPrice()
    {
        ...
        exit(json_encode(
            [
                ...
                'display_product_price' => WkMpHelper::displayPrice($productPrice, $adminDefaultCurrency),
                'display_productTI_price' => WkMpHelper::displayPrice($productTIPrice, $adminDefaultCurrency),
            ]
        ));
    ...
    public static function validateMpProductForm()
    {
        ...
        // Check fields sizes
        $rules = call_user_func(['Product', 'getValidationRules'], 'Product');
        ...
        foreach ($languages as $language) {
            ...
            if (!Validate::isCatalogName(Tools::getValue('product_name_' . $language['id_lang']))) {
                ...
                $wkErrors[] = sprintf($objMp->l('The Product Name field is too long (%2$d chars max).', $className), call_user_func([$className, 'displayFieldName'], $className), $rules['sizeLang']['name']);
            }
            ...
            if (Tools::getValue('available_now_' . $language['id_lang'])) {
                if (!Validate::isGenericName(Tools::getValue('available_now_' . $language['id_lang']))) {
                    ...
                    $wkErrors[] = sprintf($objMp->l('Label when in stock field is too long (%2$d chars max).', $className), call_user_func([$className, 'displayFieldName'], $className), $rules['sizeLang']['available_now']);
                }
            }
            if (Tools::getValue('available_later_' . $language['id_lang'])) {
                if (!Validate::isGenericName(Tools::getValue('available_later_' . $language['id_lang']))) {
                    ...
                    $wkErrors[] = sprintf($objMp->l('Label when out of stock field is too long (%2$d chars max).', $className), call_user_func([$className, 'displayFieldName'], $className), $rules['sizeLang']['available_later']);
                }
            }
            ...
            if (Tools::getValue('delivery_in_stock_' . $language['id_lang'])) {
                if (!Validate::isGenericName(Tools::getValue('delivery_in_stock_' . $language['id_lang']))) {
                    ...
                    $wkErrors[] = sprintf($objMp->l('Delivery time of in-stock products field is too long (%2$d chars max).', $className), call_user_func([$className, 'displayFieldName'], $className), $rules['sizeLang']['delivery_in_stock']);
                }
            }
            if (Tools::getValue('delivery_out_stock_' . $language['id_lang'])) {
                if (!Validate::isGenericName(Tools::getValue('delivery_out_stock_' . $language['id_lang']))) {
                    ...
                    $wkErrors[] = sprintf($objMp->l('Delivery time of out-of-stock products field is too long (%2$d chars max).', $className), call_user_func([$className, 'displayFieldName'], $className), $rules['sizeLang']['delivery_out_stock']);
                }
            }
            ...
            if (Tools::getValue('meta_title_' . $language['id_lang'])) {
                if (!Validate::isGenericName(Tools::getValue('meta_title_' . $language['id_lang']))) {
                    ...
                    $wkErrors[] = sprintf($objMp->l('Product meta title field is too long (%2$d chars max).', $className), call_user_func([$className, 'displayFieldName'], $className), $rules['sizeLang']['meta_title']);
                }
            }
            if (Tools::getValue('meta_description_' . $language['id_lang'])) {
                if (!Validate::isGenericName(Tools::getValue('meta_description_' . $language['id_lang']))) {
                    ...
                    $wkErrors[] = sprintf($objMp->l('Product meta description field is too long (%2$d chars max).', $className), call_user_func([$className, 'displayFieldName'], $className), $rules['sizeLang']['meta_description']);
                }
            }
            if (Tools::getValue('link_rewrite_' . $language['id_lang'])) {
                if (!Validate::isGenericName(Tools::getValue('link_rewrite_' . $language['id_lang']))) {
                    ...
                    $wkErrors[] = sprintf($objMp->l('Product friendly url field is too long (%2$d chars max).', $className), call_user_func([$className, 'displayFieldName'], $className), $rules['sizeLang']['link_rewrite']);
                }
            }
        }
        ...
        if ($productType == 2) {
            ...
            if ($_FILES['mp_vrt_prod_file']['size']) {
                $fileMimeType = @mime_content_type($_FILES['mp_vrt_prod_file']['tmp_name']);
                $knownMimeTypes = WkMpHelper::getAllMimeType();
                if (!in_array($fileMimeType, $knownMimeTypes)) { // Checked mime type
                    $wkErrors[] = $objMp->l('Uploaded file is not valid.', $className);
                } elseif ($_FILES['mp_vrt_prod_file']['size'] > $allowedSize) {
                    $wkErrors[] = sprintf(
                        $objMp->l('Uploaded file size must be less than %s MB.', $className),
                        Configuration::get('PS_ATTACHMENT_MAXIMUM_SIZE')
                    );
                } elseif ($mpVirtualProductName && !Validate::isCleanHtml($mpVirtualProductName)) {
                ...
                if ($mpVirtualProductName && !Validate::isCleanHtml($mpVirtualProductName)) {
                    $wkErrors[] = $objMp->l('Please enter valid file name.', $className);
                } elseif (Tools::strlen($mpVirtualProductName) > 32) {
                    $wkErrors[] = sprintf($objMp->l('The filename is too long (%2d chars max).', $className), 32);
                }
        ...
        if ($location = Tools::getValue('location')) {
            if (!empty($location)) {
                if (!Validate::isString($location) || WkMpHelper::checkScriptInHtml($location)) {
    ...
    public static function validationProductFormField($params)
    {
        ...
        foreach ($languages as $language) {
            ...
            // Product SEO Validation
            if ($params['meta_title_' . $language['id_lang']]) {
                if (!Validate::isGenericName($params['meta_title_' . $language['id_lang']])) {
                    $msg = sprintf($objMp->l('Product meta title field %s is invalid.', $className), $language['name']);
                    $data = [
                        'status' => 'ko',
                        'tab' => 'wk-seo',
                        'multilang' => '1',
                ...
                } elseif (Tools::strlen($params['meta_title_' . $language['id_lang']]) > $rules['sizeLang']['meta_title']) {
                    $msg = sprintf($objMp->l('Product meta title field is too long (%2$d chars max).', $className), call_user_func([$className, 'displayFieldName'], $className), $rules['sizeLang']['meta_title']);
                    $data = [
                        'status' => 'ko',
                        'tab' => 'wk-seo',
                        'multilang' => '1',
            ...
            if ($params['meta_description_' . $language['id_lang']]) {
                if (!Validate::isGenericName($params['meta_description_' . $language['id_lang']])) {
                    $msg = sprintf($objMp->l('Product meta description field %s is invalid.', $className), $language['name']);
                    $data = [
                        'status' => 'ko',
                        'tab' => 'wk-seo',
                        'multilang' => '1',
                    ...
                    exit(json_encode($data));
                } elseif (Tools::strlen($params['meta_description_' . $language['id_lang']]) > $rules['sizeLang']['meta_description']) {
                    $msg = sprintf($objMp->l('Product meta description field is too long (%2$d chars max).', $className), call_user_func([$className, 'displayFieldName'], $className), $rules['sizeLang']['meta_description']);
                    $data = [
                        'status' => 'ko',
                        'tab' => 'wk-seo',
                        'multilang' => '1',
            ...
            if ($params['link_rewrite_' . $language['id_lang']]) {
                if (!Validate::isGenericName($params['link_rewrite_' . $language['id_lang']])) {
                    $msg = sprintf($objMp->l('Product friendly url field %s is invalid.', $className), $language['name']);
                    $data = [
                        'status' => 'ko',
                        'tab' => 'wk-seo',
                        'multilang' => '1',
                    ...
                    exit(json_encode($data));
                } elseif (Tools::strlen($params['link_rewrite_' . $language['id_lang']]) > $rules['sizeLang']['link_rewrite']) {
                    $msg = sprintf($objMp->l('Product friendly url field is too long (%2$d chars max).', $className), call_user_func([$className, 'displayFieldName'], $className), $rules['sizeLang']['link_rewrite']);
                    $data = [
                        'status' => 'ko',
                        'tab' => 'wk-seo',
                        'multilang' => '1',
        ...
        if (!$params['product_name_' . $defaultLang]) {
            ...
            $data = [
                ...
                'multilang' => '1',
                'inputName' => 'product_name_all',
                'msg' => $objMp->l('Product name has invalid characters.', $className),
            ];
            exit(json_encode($data));
        }
        ...
        if (isset($params['product_type'])) {
            ...
            if ($productType == 2) {
                ...
                if ($mpVirtualProductName && !Validate::isCleanHtml($mpVirtualProductName)) {
                    ...
                    exit(json_encode($data));
                } elseif ($mpVirtualProductName && Tools::strlen($mpVirtualProductName) > 32) {
                    $data['inputName'] = 'mp_vrt_prod_name';
        ...
        if (Configuration::get('WK_MP_PRODUCT_MPN')) {
            if (isset($params['mpn'])) {
                $mpn = $params['mpn'];
                if (!empty($mpn)) {
                    if (!Validate::isString($mpn) || WkMpHelper::checkScriptInHtml($mpn)) {
                        $data = [
                            'status' => 'ko',
                            'tab' => 'wk-options',
                            'multilang' => '0',
                            'inputName' => 'mpn',
                            'msg' => $objMp->l('MPN value is too long. It should have 40 character or less.', $className),
                        ];
                        exit(json_encode($data));
                    }
                }
        ...
        Hook::exec('actionMpFormValidation', ['params' => $params]);
    }
    ...
    {
        if (!Validate::isLoadedObject(new Product($idPsProduct))) {
            return false;
        }
            ...
            if ($idMpProduct = $objSellerProduct->id) {
                if (Configuration::get('WK_MP_SELLER_SHIPPING')) {
                    ...
                    if (Configuration::get('WK_MP_VIRTUAL_PRODUCT')) {
                        ...
                        if ($objMvp->isMpProductIsVirtualProduct($idMpProduct)) {
                            $updateAllowed = false;
                        }
                    }
                    if ($updateAllowed) {
                        $objMpSellerProduct = new WkMpSellerShipping();
                        $mpSellerShippingDetails = $objMpSellerProduct->getDefaultShippingBySellerId($idSeller);
                        if ($mpSellerShippingDetails) {
                            ...
                            WkMpSellerShipping::deletePsProductCarrier($idPsProduct);

                            // set carrier using carrier reference
                            $objWkMpShipping = new WkMpSellerShipping();
                            $objWkMpShipping->assignAdminDefaultCarriersToSellerProduct($idPsProduct);
                        }
                    }
                }
    ...
    public static function getSellerAllShopProduct(
        ...
        $sql = 'SELECT *, msp.`date_add` as `date_add`, msp.`date_upd` as `date_upd`
    ...
    {
        if ($mpProductId) {
            if ($productDetail = WkMpSellerProduct::getSellerProductByIdProduct($mpProductId)) {
                if ($psProductId = $productDetail['id_ps_product']) {
                    $customFields = Tools::getValue('custom_fields');
                    if (!$customFields) {
                        $customFields = [];
                    }
                    $this->insertIntoPsProductCustomization(
                        $mpProductId,
                        $psProductId,
                        $customFields
                    );
