...
class WkMpSellerProductImage
{
    public static function uploadImage($files, $actionIdForUpload, $adminupload)
    {
        ...
        $data = $uploader->upload($imageFiles, [
            ...
            'extensions' => ['jpg', 'png', 'gif', 'jpeg'], // Whitelist for file extension.
        ...
        if ($data['hasErrors']) {
            ...
            if ($data['data']['metas'][0]['name']) {
                ...
                if ($actionType == 'productimage') {
                    ...
                    if ($objMpSeller->save()) {
                        $finalResult = true;
                        $finalData['sellerprofileimage'] = 1;
                    }
        ...
        if ($psIdProduct = WkMpSellerProduct::getPsIdProductByMpIdProduct($mpIdProduct)) {
            ...
            if ($productImages) {
                ...
                foreach ($productImages as &$image) {
                    ...
                    $image['image_large_link'] = $context->link->getImageLink(
                        $productLinkRewrite,
                        $psIdProduct . '-' . $image['id_image'],
    ...
    public static function getProductCoverImage($mpIdProduct, $psIdProduct = false)
    {
        ...
        if (!$coverImage && (new ShopGroup(Context::getContext()->shop->id_shop_group))->share_customer) {
            $sql = 'SELECT image_shop.`id_image`
                    FROM `' . _DB_PREFIX_ . 'image` i
                    INNER JOIN ' . _DB_PREFIX_ . 'image_shop image_shop ON (image_shop.id_image = i.id_image)
                    WHERE i.`id_product` = ' . (int) $psIdProduct . '
                    AND image_shop.`cover` = 1';
            $coverImage = Db::getInstance()->getRow($sql);
        }
    ...
    public static function setProductCoverImage($mpIdProduct, $idImage)
    {
        if ($psIdProduct = WkMpSellerProduct::getPsIdProductByMpIdProduct($mpIdProduct)) {
            ...
            $image->cover = 1;
            if (is_null($image->id_product) || $image->update()) {
