...
<div class="panel">
	...
	<div class="panel-body">
					...
					{if isset($seller_list)}
						<select name="mp_customer_id">
							{foreach $seller_list as $seller}
								<option value="{$seller['seller_customer_id']|escape:'htmlall':'UTF-8'}" {if isset($smarty.post.mp_customer_id)}{if $smarty.post.mp_customer_id == $seller['seller_customer_id']}Selected="Selected"{/if}{/if}>
				...
				<div class="col-lg-6">
					{foreach from=$languages item=language}
						...
						<div id="desc_div_{$language.id_lang|escape:'htmlall':'UTF-8'}" class="desc_div_all" {if $current_lang.id_lang != $language.id_lang}style="display:none;"{/if}>
							...
							id="description_{$language.id_lang|escape:'htmlall':'UTF-8'}" cols="2" rows="3" class="suppdesc wk_tinymce form-control">{if isset($supplier_info)}{$supplier_info.description[{$language.id_lang|escape:'htmlall':'UTF-8'}]|escape:'htmlall':'UTF-8'}{/if}</textarea>
						</div>
					{/foreach}
		  		</div>
			</div>
		...
		<div class="form-group">
			...
			<div class="col-lg-5">
				{if isset($supplier_image)}
					<img class="img-thumbnail" src="{$supplier_image|escape:'htmlall':'UTF-8'}" width="50" style="margin-bottom:5px;" />
				...
				<div class="col-lg-6">
					{foreach from=$languages item=language}
						...
						<div id="meta_title_div_{$language.id_lang|escape:'htmlall':'UTF-8'}" class="meta_title_div_all" {if $current_lang.id_lang != $language.id_lang}style="display:none;"{/if}>
							...
							class="form-control"
							value="{if isset($supplier_info)}{$supplier_info.meta_title[{$language.id_lang|escape:'htmlall':'UTF-8'}]|escape:'htmlall':'UTF-8'}{/if}">
						</div>
					{/foreach}
		  		</div>
			</div>
				...
				<div class="col-lg-6">
					{foreach from=$languages item=language}
						...
						<div id="meta_desc_div_{$language.id_lang|escape:'htmlall':'UTF-8'}" class="meta_desc_div_all" {if $current_lang.id_lang != $language.id_lang}style="display:none;"{/if}>
							...
							class="form-control"
							value="{if isset($supplier_info)}{$supplier_info.meta_description[{$language.id_lang|escape:'htmlall':'UTF-8'}]|escape:'htmlall':'UTF-8'}{/if}">
						</div>
					{/foreach}
		  		</div>
			</div>
				...
				<div class="col-lg-6">
					{foreach from=$languages item=language}
						...
						<div id="meta_key_div_{$language.id_lang|escape:'htmlall':'UTF-8'}" class="meta_desc_div_all" {if $current_lang.id_lang != $language.id_lang}style="display:none;"{/if}>
							...
							class="form-control"
							value="{if isset($supplier_info)}{$supplier_info.meta_keywords[{$language.id_lang|escape:'htmlall':'UTF-8'}]|escape:'htmlall':'UTF-8'}{/if}">
						</div>
					{/foreach}
		  		</div>
			</div>
		...
		<div class="form-group" id='dni_required'>
			<div class="row">
				...
				<div class="col-lg-6">
					<input type="text" class="form-control" placeholder="{l s='DNI' mod='marketplace'}" name="dni" id="dni" value="{if isset($smarty.post.dni)}{$smarty.post.dni|escape:'htmlall':'UTF-8'}{else}{if isset($supplier_info.dni)}{$supplier_info.dni|escape:'htmlall':'UTF-8'}{/if}{/if}" />
				</div>
			</div>
		</div>
	...
	<div class="panel-footer">
		<a href="{$link->getAdminLink('AdminMpSuppliers')|escape:'htmlall':'UTF-8'}" class="btn btn-default">
			<i class="process-icon-cancel"></i>
			{l s='Cancel' mod='marketplace'}
		</a>
		<button type="submit" name="submitAdd{$table|escape:'htmlall':'UTF-8'}" class="btn btn-default pull-right" {if $is_form_submit == 0} disabled="disabled" {/if}>
			<i class="process-icon-save"></i>
			{l s='Save' mod='marketplace'}
		</button>
		<button type="submit" name="submitAdd{$table|escape:'htmlall':'UTF-8'}AndStay" class="btn btn-default pull-right" {if $is_form_submit == 0} disabled="disabled" {/if}>
			<i class="process-icon-save"></i>
			{l s='Save and stay' mod='marketplace'}
		</button>
	</div>
	</form>
</div>
