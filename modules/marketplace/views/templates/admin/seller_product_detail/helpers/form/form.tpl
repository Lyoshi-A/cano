	...
	{if isset($mp_sellers)}
		{if isset($ps_products) && $ps_products}
			<form id="wk_mp_seller_assign_product_form" method="post"
				action="{$current|escape:'htmlall':'UTF-8'}&{if !empty($submit_action)}{$submit_action|escape:'htmlall':'UTF-8'}{/if}&token={$token|escape:'htmlall':'UTF-8'}&assignmpproduct=1"
				class="defaultForm form-horizontal {$name_controller|escape:'htmlall':'UTF-8'}" enctype="multipart/form-data"
				...
				<div class="panel">
							...
							<div class="col-lg-3">
								<select name="id_customer">
									{foreach $mp_sellers as $seller}
										<option value="{$seller.id_customer|escape:'htmlall':'UTF-8'}">
											{$seller.business_email|escape:'htmlall':'UTF-8'}
											{if isset($all_shop) && $all_shop && isset($seller.ps_shop_name)}
												({$seller.ps_shop_name|escape:'htmlall':'UTF-8'})
											{/if}
										</option>
									{/foreach}
								</select>
					...
					<div class="panel-footer">
						<a href="{$link->getAdminLink('AdminSellerProductDetail')|escape:'htmlall':'UTF-8'}" class="btn btn-default"><i
								class="process-icon-cancel"></i> {l s='Cancel' mod='marketplace'}</a>
						<button type="submit" name="submitAdd{$table|escape:'htmlall':'UTF-8'}" class="btn btn-default pull-right wk-prod-assign"><i
								class="process-icon-save"></i> {l s='Assign' mod='marketplace'}</button>
						<button type="submit" name="submitAdd{$table|escape:'htmlall':'UTF-8'}AndAssignStay"
...
{else}
	<div class="panel">
		...
		<form name="mp_admin_saveas_button" id="mp_admin_saveas_button"
			class="defaultForm {$name_controller|escape:'htmlall':'UTF-8'} form-horizontal"
			action="{if isset($edit)}{$current|escape:'htmlall':'UTF-8'}&update{$table|escape:'htmlall':'UTF-8'}&id_mp_product={$product_info.id_mp_product|escape:'htmlall':'UTF-8'}&token={$token|escape:'htmlall':'UTF-8'}{else}{$current|escape:'htmlall':'UTF-8'}&add{$table|escape:'htmlall':'UTF-8'}&token={$token|escape:'htmlall':'UTF-8'}{/if}"
					...
					{else}
						<div class="form-group">
							<label class="control-label">
								&nbsp;&nbsp;{l s='Unique shop name -' mod='marketplace'}
								<span>{$shop_name_unique|escape:'htmlall':'UTF-8'}</label>
							</label>
						</div>
					{/if}
					{if $multi_lang}
						<div class="form-group">
							<label class="control-label">
								...
								<span id="seller_default_lang_div">{$current_lang.name|escape:'htmlall':'UTF-8'}</label>
							</label>
						</div>
					{/if}
				</div>
							...
							{foreach from=$languages item=language}
								<li>
									<a href="javascript:void(0)"
										onclick="showProdLangField('{$language.name|escape:'htmlall':'UTF-8'}', {$language.id_lang|escape:'htmlall':'UTF-8'});">
			...
			<input type="hidden" value="{if isset($edit)}{$product_info.id_mp_product|escape:'htmlall':'UTF-8'}{/if}" name="id" id="mp_product_id" />
			<input type="hidden" name="seller_default_lang" id="seller_default_lang" value="{$current_lang.id_lang|escape:'htmlall':'UTF-8'}">
				...
				<div class="tab-content panel collapse in">
					<div class="tab-pane active" id="wk-information">
							...
							<div class="col-lg-6">
								{foreach from=$languages item=language}
									...
									<input type="text" id="product_name_{$language.id_lang|escape:'htmlall':'UTF-8'}"
										...
										value="{if isset($smarty.post.$product_name)}{$smarty.post.$product_name|escape:'htmlall':'UTF-8'}{elseif isset($edit)}{$product_info.name[{$language.id_lang|escape:'htmlall':'UTF-8'}]|escape:'htmlall':'UTF-8'}{/if}"
						...
						<div class="row pkprod_container">
							<input type="hidden" name="current_lang_id" id="current_lang_id"
								value="{$current_lang.id_lang|escape:'htmlall':'UTF-8'}">
							<div class="col-sm-12">
								<div class="form-group">
									<label class="col-sm-3 text-right">{l s='Select Pack Products' mod='marketplace'}
										<img class="wk_pack_product_loader"
											src="{$module_dir|escape:'htmlall':'UTF-8'}marketplace/views/img/loader.gif" width="20" />
									</label>
								</div>
								<div class="form-group">
									...
									<div class="col-sm-offset-3 col-sm-9">
										<button class="btn btn-info" id="addpackprodbut">
											<span> <i class="icon-plus-sign-alt"></i>
												{l s='Add this product to pack' mod='marketplace'} </span>
										</button>
									</div>
								</div>
												...
												{foreach from=$mpPackProducts key=k item=mpPackProduct}
													<div class="col-sm-4 col-xs-12">
														<div class="row no_margin pk_sug_prod"
															ps_prod_id="{$mpPackProduct->id_ps_product|escape:'htmlall':'UTF-8'}"
															ps_id_prod_attr="{$mpPackProduct->ps_prod_attr_id|escape:'htmlall':'UTF-8'}">
															<div class="col-sm-12 col-xs-12">
																<img class="img-responsive pk_sug_img"
																	src="{$mpPackProduct->image_link|escape:'htmlall':'UTF-8'}">
																<p class="text-center">{$mpPackProduct->product_name|escape:'htmlall':'UTF-8'}</p>
																{if $mpPackProduct->product_ref != ''}
																	<p class="text-center">{l s='REF:' mod='marketplace'}
																		{$mpPackProduct->product_ref|escape:'htmlall':'UTF-8'}</p>
																{/if}
																<span class="pull-left">x{$mpPackProduct->quantity|escape:'htmlall':'UTF-8'}</span>
																<a class="pull-right dltpkprod">
																	<i class="material-icons">&#xE872;</i></a>
																<input type="hidden" class="pspk_id_prod" name="pspk_id_prod[]"
																	value="{$mpPackProduct->id_ps_product|escape:'htmlall':'UTF-8'}">
																<input type="hidden" name="pspk_prod_quant[]"
																	value="{$mpPackProduct->quantity|escape:'htmlall':'UTF-8'}">
																<input type='hidden' class='pspk_id_prod_attr'
																	name='pspk_id_prod_attr[]'
																	value="{$mpPackProduct->ps_prod_attr_id|escape:'htmlall':'UTF-8'}">
															</div>
														</div>
													</div>
												{/foreach}
								...
								{foreach from=$languages item=language}
									...
									<div id="short_desc_div_{$language.id_lang|escape:'htmlall':'UTF-8'}"
										...
										<textarea name="short_description_{$language.id_lang|escape:'htmlall':'UTF-8'}"
											...
											class="wk_tinymce form-control">{if isset($smarty.post.$short_desc_name)}{$smarty.post.$short_desc_name|escape:'htmlall':'UTF-8'}{elseif isset($edit)}{$product_info.description_short[{$language.id_lang|escape:'htmlall':'UTF-8'}]|escape:'htmlall':'UTF-8'}{/if}</textarea>
									</div>
								{/foreach}
							</div>
						</div>
							...
							<div class="col-lg-6">
								{foreach from=$languages item=language}
									...
									<div id="product_desc_div_{$language.id_lang|escape:'htmlall':'UTF-8'}"
										class="wk_text_field_all wk_text_field_{$language.id_lang|escape:'htmlall':'UTF-8'}"
										{if $current_lang.id_lang != $language.id_lang}style="display:none;" {/if}>
										<textarea name="description_{$language.id_lang|escape:'htmlall':'UTF-8'}" id="description_{$language.id_lang|escape:'htmlall':'UTF-8'}"
											cols="2" rows="3"
											class="wk_tinymce form-control">{if isset($smarty.post.$description)}{$smarty.post.$description|escape:'htmlall':'UTF-8'}{elseif isset($edit)}{$product_info.description[{$language.id_lang|escape:'htmlall':'UTF-8'}]|escape:'htmlall':'UTF-8'}{/if}</textarea>
									</div>
								{/foreach}
							</div>
						</div>
						<div class="form-group">
							...
							<div class="col-lg-6">
								<input type="text" class="form-control" name="reference" id="reference"
									value="{if isset($smarty.post.reference)}{$smarty.post.reference|escape:'htmlall':'UTF-8'}{else if isset($edit)}{$product_info.reference|escape:'htmlall':'UTF-8'}{/if}"
									maxlength="32" />
							</div>
						</div>
						<div class="form-group">
					...
					<div class="form-group">
						<label class="col-lg-3 control-label required" for="product_category">
							...
							<div class="wk_tooltip">
								<span
									class="wk_tooltiptext">{l s='Where should the product be available on your site? The main category is where the product appears by default: this is the category which is seen in the product page\'s URL.' mod='marketplace'}
								</span>
							</div>
						</label>
						<div class="col-lg-6">
							...
							<input type="hidden" name="product_category" id="product_category"
								value="{if isset($catIdsJoin)}{$catIdsJoin|escape:'htmlall':'UTF-8'}{/if}" />
						</div>
					</div>
								...
								{if isset($defaultCategory)}
									{foreach $defaultCategory as $defaultCategoryVal}
										<option id="default_cat{$defaultCategoryVal.id_category|escape:'htmlall':'UTF-8'}"
											value="{$defaultCategoryVal.id_category|escape:'htmlall':'UTF-8'}"
											{if isset($defaultIdCategory)}
												{if $defaultIdCategory == $defaultCategoryVal.id_category}
												selected {/if}
											{/if}>
											{$defaultCategoryVal.name|escape:'htmlall':'UTF-8'}
										</option>
									{/foreach}
								{else}
									<option id="default_cat2" value="2">{l s='Home' mod='marketplace'}</option>
								{/if}
							</select>
						</div>
					</div>
					...
					<div class="row vir_container">
						<hr>
						<div class="row col-lg-offset-2" style="margin-left: 14.4%;">
								...
								<label class="col-sm-3 control-label"
									...
									<div class="wk_tooltip">
										<span
											...
											({Configuration::get('PS_ATTACHMENT_MAXIMUM_SIZE')|string_format:'%.2f'|escape:'htmlall':'UTF-8'}
											{l s='MB max.' mod='marketplace'})</span>
									</div>
								</label>
								<div class="col-sm-6">
									<div class="mp_virtual_upload-btn-wrapper">
										<button class="mp_vitual_btnr" type="button">{l s='Choose file' mod='marketplace'}</button>
							...
							<div class="form-group">
								...
								<div class="col-sm-6">
									<input type="text" class="form-control" name="mp_vrt_prod_name"
										id="mp_vrt_prod_name"
										{if isset($is_virtual_prod)}value="{$is_virtual_prod['display_filename']|escape:'htmlall':'UTF-8'}"
										{/if}>
								</div>
							</div>
							{if isset($is_virtual_prod) && isset($attach_file_exist) && isset($id) && isset($showTab)}
								<div class="form-group">
									<div class="col-sm-6 col-sm-offset-3">
										<a href="{$link->getModuleLink('marketplace', 'downloadFile',['id_value' => $id, 'admin' => 1])|escape:'htmlall':'UTF-8'}"
											class="btn btn-default">
											<i class="icon-download"></i>
											{l s='download file' mod='marketplace'}
										</a>
										<input type="hidden" name="mp_id_prod" id="mp_id_prod" value="{$id|escape:'htmlall':'UTF-8'}">
							...
							<div class="form-group">
								...
								<div class="col-sm-6">
									<input type="text" class="form-control" name="mp_vrt_prod_nb_downloable"
										...
										value="{if isset($is_virtual_prod)}{$is_virtual_prod['nb_downloadable']|escape:'htmlall':'UTF-8'}{else}0{/if}">
								</div>
							</div>
							<div class="form-group">
								...
								<div class="col-sm-6">
									<input type="text" autocomplete="off" class="datepicker form-control"
										...
										{if isset($is_virtual_prod)}value="{$is_virtual_prod['date_expiration']|date_format:"Y-m-d"|escape:'htmlall':'UTF-8'}"
										{/if}>
								</div>
							</div>
							<div class="form-group">
								...
								<div class="col-sm-6">
									<input type="text" class="form-control" name="mp_vrt_prod_nb_days"
										...
										value="{if isset($is_virtual_prod)}{$is_virtual_prod['nb_days_accessible']|escape:'htmlall':'UTF-8'}{else}0{/if}">
								</div>
							</div>
						</div>
					</div>
					...
					<div class="form-group">
						...
						<div class="col-lg-6">
							<input type="text" class="form-control" name="ean13" id="ean13"
								value="{if isset($smarty.post.ean13)}{$smarty.post.ean13|escape:'htmlall':'UTF-8'}{else if isset($edit)}{$product_info.ean13|escape:'htmlall':'UTF-8'}{/if}"
								maxlength="13" />
						</div>
					</div>
					<div class="form-group">
						...
						<div class="col-lg-6">
							<input type="text" class="form-control" name="upc" id="upc"
								value="{if isset($smarty.post.upc)}{$smarty.post.upc|escape:'htmlall':'UTF-8'}{else if isset($edit)}{$product_info.upc|escape:'htmlall':'UTF-8'}{/if}"
								maxlength="12" />
						</div>
					</div>
					<div class="form-group">
						...
						<div class="col-lg-6">
							<input type="text" class="form-control" name="isbn" id="isbn"
								value="{if isset($smarty.post.isbn)}{$smarty.post.isbn|escape:'htmlall':'UTF-8'}{else if isset($edit)}{$product_info.isbn|escape:'htmlall':'UTF-8'}{/if}"
								maxlength="13" />
						</div>
					</div>
						...
						<div class="form-group">
							...
							<div class="col-lg-6">
								<input type="text" class="form-control" name="mpn" id="mpn"
									value="{if isset($smarty.post.mpn)}{$smarty.post.mpn|escape:'htmlall':'UTF-8'}{else if isset($edit)}{$product_info.mpn|escape:'htmlall':'UTF-8'}{/if}"
								/>
							</div>
						</div>
					{/if}
		...
		<div class="panel-footer">
			<a href="{$link->getAdminLink('AdminSellerProductDetail')|escape:'htmlall':'UTF-8'}" class="btn btn-default">
				<i class="process-icon-cancel"></i>{l s='Cancel' mod='marketplace'}
			</a>
			<button type="submit" name="submitAdd{$table|escape:'htmlall':'UTF-8'}" class="btn btn-default pull-right" id="mp_admin_save_button">
		...
		{include file="$wkself/../../views/templates/front/product/_partials/mp-image-preview.tpl"}
	</form>
</div>
{/if}
