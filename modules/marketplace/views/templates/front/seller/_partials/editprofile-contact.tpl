...
<div class="form-group">
	...
	<div id="address_div">
		...
		class="validate form-control">{if isset($smarty.post.address)}{$smarty.post.address|escape:'htmlall':'UTF-8'}{else}{$mp_seller_info.address|escape:'htmlall':'UTF-8'}{/if}</textarea>
	</div>
</div>
...
<div class="form-group row">
	{if $seller_country_need}
		<div class="col-md-6" id="seller_zipcode">
			...
			type="text"
			value="{if isset($smarty.post.postcode)}{$smarty.post.postcode|escape:'htmlall':'UTF-8'}{else}{$mp_seller_info.postcode|escape:'htmlall':'UTF-8'}{/if}"
			name="postcode"
			id="postcode" />
		</div>
		<div class="col-md-6">
			...
			<input class="form-control"
			type="text"
			value="{if isset($smarty.post.city)}{$smarty.post.city|escape:'htmlall':'UTF-8'}{else}{$mp_seller_info.city|escape:'htmlall':'UTF-8'}{/if}"
			name="city"
			id="city"
			maxlength="64" />
		</div>
...
{if $seller_country_need && isset($wk_country)}
	<div class="form-group row">
		<div class="col-md-6">
			...
			<select name="id_country" id="id_country" class="form-control form-control-select">
				...
				{foreach $wk_country as $countrydetail}
					<option value="{$countrydetail.id_country|escape:'htmlall':'UTF-8'}"
