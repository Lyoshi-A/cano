...
{if Configuration::get('WK_MP_REVIEW_SETTINGS')}
	<div class="wk-mp-block">
		<div class="page-title" style="background-color:{$title_bg_color|escape:'htmlall':'UTF-8'};">
			<span style="color:{$title_text_color|escape:'htmlall':'UTF-8'};">{l s='All Reviews' mod='marketplace'} {if isset($nbReviews)}({$nbReviews|escape:'htmlall':'UTF-8'}){/if}</span>
		</div>
						...
						<div class="wk-pagination-right">
							<div class="col-sm-12">
								<ul class="pagination pagination-sm">
									<li class="page-item">
										<a class="page-link {if $p == 1}wk-disabled{/if}" {if $p > 1}href="{$link->getModuleLink('marketplace', 'allreviews', ['mp_shop_name' => $shopLinkRewrite, 'p' => $p-1])|escape:'htmlall':'UTF-8'}"{/if} aria-label="Previous">
											<span aria-hidden="true">{l s='Previous' mod='marketplace'}</span>
										</a>
									</li>
									{for $i = 1 to $page_count}
										<li class="page-item">
											<a class="page-link {if $p == $i}wk-page-active{/if}" {if $p != $i}href="{$link->getModuleLink('marketplace', 'allreviews', ['mp_shop_name' => $shopLinkRewrite, 'p' => $i])|escape:'htmlall':'UTF-8'}"{/if}>{$i|escape:'htmlall':'UTF-8'}</a>
										</li>
									{/for}
									<li class="page-item">
										<a class="page-link {if $p == $page_count}wk-disabled{/if}" {if $p < $page_count}href="{$link->getModuleLink('marketplace', 'allreviews', ['mp_shop_name' => $shopLinkRewrite, 'p' => $p+1])|escape:'htmlall':'UTF-8'}"{/if} aria-label="Next">
