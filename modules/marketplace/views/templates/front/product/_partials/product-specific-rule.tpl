	...
	<div class="form-group row col-md-12">
		<div class="wkslotprice {if isset($backendController)}col-lg-8 col-lg-offset-2{/if} wk_padding_none">
			...
			<div id="add_specific_price" class="clearfix" style="display:none;">
				...
				<input type="hidden" name="mp_product_id" value="{if isset($mp_product_id) && $mp_product_id}{$mp_product_id|escape:'htmlall':'UTF-8'}{/if}" id="mp_product_id">
							...
							<div class="col-lg-4">
								<select name="sp_id_currency" id="spm_currency_0" onchange="changeCurrencySpecificPrice(0);" class="form-control form-control-select wkinput">
									...
									{foreach from=$currencies item=curr}
										<option value="{$curr.id_currency|escape:'htmlall':'UTF-8'}">
											{$curr.name|escape:'htmlall':'UTF-8'}
										</option>
									{/foreach}
								</select>
							</div>
							...
							<div class="col-lg-6">
								<select name="sp_id_product_attribute" id="sp_id_product_attribute" class="form-control form-control-select wkinput">
									...
									{foreach from=$combinationDetailsWithPsIds item=sp_comb}
										<option value={$sp_comb.id_product_attribute|escape:'htmlall':'UTF-8'}>
											{$sp_comb['attribute_designation']|escape:'htmlall':'UTF-8'}
										</option>
									{/foreach}
								</select>
							</div>
				...
				<div class="form-group clearfix">
					<label class="control-label col-lg-2 wklabel required" for="sp_from_quantity">
								...
								<select name="sp_reduction_type" id="sp_reduction_type" class="form-control form-control-select wkinput">
									<option value="amount" selected="selected">
										{if isset($defaultCurrencySign) && $defaultCurrencySign}
											{$defaultCurrencySign|escape:'htmlall':'UTF-8'}
										{else}
				...
				{if isset($updateProduct) && $updateProduct == 1}
					<div class="slot-button">
						<a id="add_btn" class="btn btn-info" href="#" {if isset($controller) &&
		...
		{if isset($priceRules) && isset($updateProduct) && $updateProduct == 1}
			<div id="slot_listing" class="table-responsive {if isset($controller) &&
		$controller=='admin' }col-lg-8 col-lg-offset-2{else}col-lg-12{/if}">
					...
					<tbody>
						{if isset($priceRules)}
							{foreach $priceRules as $rule}
								<tr class="even" id="slotcontent{$rule['id']|escape:'htmlall':'UTF-8'}">
									<td>{$rule['id']|escape:'htmlall':'UTF-8'}</td>
									<td>{$rule['id_combiantion_name']|escape:'htmlall':'UTF-8'}</td>
									<td>{$rule['id_currency']|escape:'htmlall':'UTF-8'}</td>
									<td>{$rule['id_country']|escape:'htmlall':'UTF-8'}</td>
									<td>{$rule['id_group']|escape:'htmlall':'UTF-8'}</td>
									<td>{$rule['id_customer']|escape:'htmlall':'UTF-8'}</td>
									<td>{$rule['price']|escape:'htmlall':'UTF-8'}</td>
									<td>{$rule['impact']|escape:'htmlall':'UTF-8'}</td>
									<td>{$rule['period'] nofilter}</td>
									<td class="text-center">{$rule['from_quantity']|escape:'htmlall':'UTF-8'}</td>
									<td>
										<div style="display:flex;">
											<a id="{$rule['id']|escape:'htmlall':'UTF-8'}" class="slot_edit_link" name="slot_edit_link" data-toggle="modal" data-target="#slot_edit_link_modal" title={l s='Edit' mod='marketplace'} href="javascript:void(0);">
												<i class="material-icons">&#xe3c9;</i>
											</a>
											&nbsp;
											<a id="{$rule['id']|escape:'htmlall':'UTF-8'}" name="slot_delete_link" data-toggle="tooltip" data-placement="top" title={l s='Delete' mod='marketplace'} href="javascript:void(0);">
												<i class="material-icons">&#xE872;</i>
											</a>
										</div>
									</td>
			...
			<div class="wkslotprice_loader">
				<img src="{$modules_dir|escape:'htmlall':'UTF-8'}marketplace/views/img/loading-small.gif" class="wkslotprice-loading-img" />
			</div>
					...
					<div class="modal-content">
						...
						<div class="modal-body">
							<div id="edit_specific_price" class="clearfix">
										...
										<div class="form-group clearfix">
											...
											<div class="col-lg-6">
												<select name="sp_id_product_attribute" id="sp_id_product_attribute_edit"
													...
													{foreach from=$combinationDetailsWithPsIds item=sp_comb}
														<option value={$sp_comb.id_product_attribute|escape:'htmlall':'UTF-8'}>{$sp_comb['attribute_designation']|escape:'htmlall':'UTF-8'}
														</option>
													{/foreach}
												</select>
											</div>
								...
								<div class="form-group clearfix">
									<label class="control-label col-lg-2 wklabel required" for="sp_from_quantity_edit">
										{l s='Starting at' mod='marketplace'}
									</label>
									<div class="col-lg-4">
										<div class="input-group">
