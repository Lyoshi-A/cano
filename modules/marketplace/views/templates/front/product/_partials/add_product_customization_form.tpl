					...
					{foreach $customizationFields as $fieldrow => $fields}
						<div class="form-group row">
							<div class="col-md-4">
								...
								<input type="hidden" name="custom_fields[{$fieldrow|escape:'htmlall':'UTF-8'}][id_customization_field]"
									id="custom_fields_{$fieldrow|escape:'htmlall':'UTF-8'}_id_customization_field" value="{$fieldrow|escape:'htmlall':'UTF-8'}">
								<div>
									<div class="">
									{foreach from=$languages item=lang}
										{assign var="texttitle" value="textname_`$lang.id_lang`"}
										<div class="wk_text_field_all wk_text_field_{$lang.id_lang|escape:'htmlall':'UTF-8'}"
											{if $current_lang.id_lang != $lang.id_lang}style="display:none;" {/if}>
											{if isset($fields[$lang.id_lang])}
												<input type="text"
													name="custom_fields[{$fieldrow|escape:'htmlall':'UTF-8'}][label][{$lang.id_lang|escape:'htmlall':'UTF-8'}]"
													id="custom_fields_{$fieldrow|escape:'htmlall':'UTF-8'}_label_{$lang.id_lang|escape:'htmlall':'UTF-8'}"
													class="form-control"
													value="{$fields[$lang.id_lang]['name']|escape:'htmlall':'UTF-8'}"
													placeholder="{l s='Field Label' mod='marketplace'}">
											{else}
												<input type="text"
													name="custom_fields[{$fieldrow|escape:'htmlall':'UTF-8'}][label][{$lang.id_lang|escape:'htmlall':'UTF-8'}]"
													id="custom_fields_{$fieldrow|escape:'htmlall':'UTF-8'}_label_{$lang.id_lang|escape:'htmlall':'UTF-8'}"
													class="form-control" value="" placeholder="{l s='Field Label' mod='marketplace'}">
											{/if}
										</div>
									{/foreach}
									</div>
							...
							<div class="col-md-8">
								...
								<div class="col-md-5 col-xs-10">
									<select id="custom_fields_{$fieldrow|escape:'htmlall':'UTF-8'}_type" name="custom_fields[{$fieldrow|escape:'htmlall':'UTF-8'}][type]"
								...
								<div class="col-md-5">
									<div class="checkbox">
										<label for="required">
											<input type="checkbox" id="custom_fields_{$fieldrow|escape:'htmlall':'UTF-8'}_required" value="1"
												{if $fields[$current_lang.id_lang]['required'] == 1}checked{/if}
												name="custom_fields[{$fieldrow|escape:'htmlall':'UTF-8'}][required]">
