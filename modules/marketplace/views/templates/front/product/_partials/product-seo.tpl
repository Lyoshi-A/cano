...
{if Configuration::get('WK_MP_SELLER_PRODUCT_SEO') || isset($backendController)}
	<div class="form-group">
		...
		{foreach from=$languages item=language}
			...
			<div id="meta_title_div_{$language.id_lang|escape:'htmlall':'UTF-8'}"
				...
				<input type="text" name="meta_title_{$language.id_lang|escape:'htmlall':'UTF-8'}" id="meta_title_{$language.id_lang|escape:'htmlall':'UTF-8'}"
					class="form-control" maxlength="128"
					value="{if isset($smarty.post.$meta_title)}{$smarty.post.$meta_title|escape:'htmlall':'UTF-8'}{else}{if isset($product_info)}{$product_info.meta_title[{$language.id_lang|escape:'htmlall':'UTF-8'}]|escape:'htmlall':'UTF-8'}{/if}{/if}"
	...
	<div class="form-group">
		...
		{foreach from=$languages item=language}
			...
			<div id="meta_description_div_{$language.id_lang|escape:'htmlall':'UTF-8'}"
				class="wk_text_field_all wk_text_field_{$language.id_lang|escape:'htmlall':'UTF-8'} {if $current_lang.id_lang != $language.id_lang}wk_display_none{/if}">
				<textarea name="meta_description_{$language.id_lang|escape:'htmlall':'UTF-8'}" id="meta_description_{$language.id_lang|escape:'htmlall':'UTF-8'}"
					class="form-control" cols="2" rows="3" maxlength="255"
					placeholder="{l s='To have a different description than your product summary in search results pages, write it here.' mod='marketplace'}">{if isset($smarty.post.$meta_description)}{$smarty.post.$meta_description|escape:'htmlall':'UTF-8'}{else}{if isset($product_info)}{$product_info.meta_description[{$language.id_lang|escape:'htmlall':'UTF-8'}]|escape:'htmlall':'UTF-8'}{/if}{/if}</textarea>
			</div>
		{/foreach}
	</div>
	<div class="form-group">
		...
		{foreach from=$languages item=language}
			...
			<div id="link_rewrite_div_{$language.id_lang|escape:'htmlall':'UTF-8'}"
				class="wk_text_field_all wk_text_field_{$language.id_lang|escape:'htmlall':'UTF-8'} {if $current_lang.id_lang != $language.id_lang}wk_display_none{/if}">
				<input type="text" name="link_rewrite_{$language.id_lang|escape:'htmlall':'UTF-8'}" id="link_rewrite_{$language.id_lang|escape:'htmlall':'UTF-8'}"
					class="form-control" maxlength="128"
					value="{if isset($smarty.post.$link_rewrite)}{$smarty.post.$link_rewrite|escape:'htmlall':'UTF-8'}{else}{if isset($product_info)}{$product_info.link_rewrite[{$language.id_lang|escape:'htmlall':'UTF-8'}]|escape:'htmlall':'UTF-8'}{/if}{/if}">
			</div>
		{/foreach}
	</div>
{/if}
{if Configuration::get('WK_MP_PRODUCT_PAGE_REDIRECTION') || isset($backendController)}
	<div class="form-group row">
		...
		<input type="hidden" name="id_type_redirected" value="{if isset($product_info) && $product_info.id_type_redirected}{$product_info.id_type_redirected|escape:'htmlall':'UTF-8'}{/if}" id="id_type_redirected">
		<div class="col-md-6">
			<div class="target_category_div {if !isset($product_info) || ($product_info.redirect_type != '301-category' && $product_info.redirect_type != '302-category')} wk_display_none {/if}">
				...
				<select class="form-control form-control-select chosen" name="target_category" id="target_category">
					...
					{if isset($redirectCategories) && $redirectCategories}
						{foreach item=category from=$redirectCategories}
							<option id="target_category{$category.id_category|escape:'htmlall':'UTF-8'}" value="{$category.id_category|escape:'htmlall':'UTF-8'}" {if isset($product_info.id_type_redirected)}{if $product_info.id_type_redirected == $category.id_category} selected {/if}{/if}>{$category.name|escape:'htmlall':'UTF-8'}</option>
						{/foreach}
					{/if}
				</select>
			...
			<div class="target_product_div {if !isset($product_info) || ($product_info.redirect_type != '301-product' && $product_info.redirect_type != '302-product')} wk_display_none {/if}">
				...
				<select class="form-control form-control-select chosen" name="target_product" id="target_product">
					...
					{if isset($redirectProducts) && $redirectProducts}
						{foreach item=redirectProduct from=$redirectProducts}
							{if isset($redirectProduct.id_product)}
								<option id="target_product{$redirectProduct.id_product|escape:'htmlall':'UTF-8'}" value="{$redirectProduct.id_product|escape:'htmlall':'UTF-8'}" {if isset($product_info.id_type_redirected)}{if $product_info.id_type_redirected == $redirectProduct.id_product} selected {/if}{/if}>
									{$redirectProduct.name|escape:'htmlall':'UTF-8'}
								</option>
							{/if}
						{/foreach}
					{/if}
