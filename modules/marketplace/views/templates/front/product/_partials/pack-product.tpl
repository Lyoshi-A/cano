...
<div class="form-group pkprod_container">
    ...
    <div class="form-group">
        ...
        <img class="wk_pack_product_loader" src="{$module_dir|escape:'htmlall':'UTF-8'}marketplace/views/img/loader.gif" width="20" />
        </label>
    ...
    <div class="form-group">
        <div class="col-sm-offset-3">
            <button class="btn btn-info" id="addpackprodbut" type="button">
        ...
        {if isset($isPackProduct) && isset($mpPackProducts) && $mpPackProducts}
            {foreach from=$mpPackProducts key=k item=mpPackProduct}
                <div class="col-sm-4 col-xs-12">
                    <div class="row no_margin pk_sug_prod" ps_prod_id="{$mpPackProduct->id_ps_product|escape:'htmlall':'UTF-8'}" ps_id_prod_attr="{$mpPackProduct->ps_prod_attr_id|escape:'htmlall':'UTF-8'}">
                        <div class="col-sm-12 col-xs-12">
                            <img class="img-responsive pk_sug_img" src="{$mpPackProduct->image_link|escape:'htmlall':'UTF-8'}">
                            <p class="text-center">{$mpPackProduct->product_name|escape:'htmlall':'UTF-8'}</p>
                            {if $mpPackProduct->product_ref != ''}
                                <p class="text-center">{l s='REF:' mod='marketplace'} {$mpPackProduct->product_ref|escape:'htmlall':'UTF-8'}</p>
                            {/if}
                            ...
                            <input type="hidden" class="pspk_id_prod" name="pspk_id_prod[]" value="{$mpPackProduct->id_ps_product|escape:'htmlall':'UTF-8'}">
                            <input type="hidden" name="pspk_prod_quant[]" value="{$mpPackProduct->quantity|escape:'htmlall':'UTF-8'}">
                            <input type='hidden' class='pspk_id_prod_attr' name='pspk_id_prod_attr[]' value="{$mpPackProduct->ps_prod_attr_id|escape:'htmlall':'UTF-8'}">
                        </div>
                    </div>
                </div>
            {/foreach}
