...
<div class="vir_container no_margin">
    ...
    <div class="form-group">
        <label class="control-label" >{l s='Upload File' mod='marketplace'}</label>
        <div class="wk_tooltip">
            <div class="wk_tooltiptext">
                {l s='Upload a file from your computer' mod='marketplace'} ({Configuration::get('PS_ATTACHMENT_MAXIMUM_SIZE')|string_format:'%.2f'|escape:'htmlall':'UTF-8'} {l s='MB max.' mod='marketplace'})
            </div>
        </div>
    ...
    <div class="form-group">
        ...
        <input type="text" class="form-control" name="mp_vrt_prod_name" id="mp_vrt_prod_name" {if isset($is_virtual_prod)}value="{$is_virtual_prod['display_filename']|escape:'htmlall':'UTF-8'}"{/if}>
    </div>
    {if isset($is_virtual_prod) && isset($attach_file_exist) && isset($id) && isset($showTab)}
    <div class="form-group">
        <a
            href="{$link->getModuleLink('marketplace', 'downloadFile',['id_value'=>$id])|escape:'htmlall':'UTF-8'}"
    ...
    <div class="form-group">
        ...
        <input type="text" class="form-control" name="mp_vrt_prod_nb_downloable" id="mp_vrt_prod_nb_downloable" value="{if isset($is_virtual_prod)}{$is_virtual_prod['nb_downloadable']|escape:'htmlall':'UTF-8'}{else}0{/if}">
    </div>
    <div class="form-group">
        ...
        {if isset($is_virtual_prod)}value="{$is_virtual_prod['date_expiration']|date_format:"Y-m-d"|escape:'htmlall':'UTF-8'}"{/if}>
    </div>
    <div class="form-group">
        ...
        <input type="text" class="form-control" name="mp_vrt_prod_nb_days" id="mp_vrt_prod_nb_days" value="{if isset($is_virtual_prod)}{$is_virtual_prod['nb_days_accessible']|escape:'htmlall':'UTF-8'}{else}0{/if}">
    </div>
</div>