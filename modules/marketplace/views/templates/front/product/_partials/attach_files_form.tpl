	...
	<div id="mp_product_attachment" class="form-group">
		...
		<div class="">
			...
			<div class='{if isset($backendController)}col-lg-6{/if}'>
				...
				<table id="product-attachment-file" class="table" {if empty($productAttachments)}style="display: none;"{/if}>
					...
					<tbody>
						{if isset($productAttachments) && $productAttachments}
							{foreach $productAttachments as $akey => $psAttachment}
								<tr>
									<td class="col-md-1">
										<input type="checkbox" class="checkbox" id="attachments_{$akey|escape:'htmlall':'UTF-8'}" name="mp_attachments[]"
											value="{$psAttachment.id_attachment|escape:'htmlall':'UTF-8'}"
											{if isset($psAttachment.selected) && $psAttachment.selected}checked{/if}>
									</td>
									<td class="col-md-3">
										<span class="">{$psAttachment.name|escape:'htmlall':'UTF-8'}</span>
									</td>
									<td class="col-md-6 file-name"><span>{$psAttachment.file_name|escape:'htmlall':'UTF-8'}</span></td>
									<td class="col-md-2 wk-attachment-file-type">{$psAttachment.mime|escape:'htmlall':'UTF-8'}</td>
								</tr>
							{/foreach}
						{/if}
					</tbody>
					...
					<div class="form-group">
						<div class="custom-file">
							...
							<div class="mp_attachment_upload-btn-wrapper">
								<button class="mp_attachment_btnr" type="button">{l s='Choose file' mod='marketplace'}</button>
