...
<div class="wk-mp-block">
	...
	<div class="wk-mp-content">
		<div class="page-title" style="background-color:{$title_bg_color|escape:'htmlall':'UTF-8'};">
			<span style="color:{$title_text_color|escape:'htmlall':'UTF-8'};">{l s='Brand Products' mod='marketplace'} - {$manuf_name|escape:'htmlall':'UTF-8'}</span>
		</div>
		<div class="wk-mp-right-column">
			...
			<p class="wk_btn_add_product wk_product_list">
				<a title="{l s='Brand List' mod='marketplace'}" href="{$link->getModuleLink('marketplace', 'mpmanufacturerlist')|escape:'htmlall':'UTF-8'}">
			...
			<table class="table">
				...
				<tbody>
					...
					{foreach $manufproductinfo as $data}
						<tr id="manufprodid_{$data.id|escape:'htmlall':'UTF-8'}">
							{if isset($manufproductinfo) && $manufproductinfo|is_array}{if $manufproductinfo|@count > 1}<td><input type="checkbox" name="mp_manf_selected[]" class="mp_bulk_select" value="{$data.id|escape:'htmlall':'UTF-8'}"/></td>{/if}{/if}
							<td>{$data.mp_product_id|escape:'htmlall':'UTF-8'}</td>
							<td>
								<a href="{$link->getModuleLink('marketplace', 'updateproduct', ['id_mp_product' => $data.mp_product_id])|escape:'htmlall':'UTF-8'}">
									{$data.product_name|escape:'htmlall':'UTF-8'}
								</a>
							</td>
							<td>{$data.quantity|escape:'htmlall':'UTF-8'}</td>
							<td>
								<a delmanufproductid="{$data.id|escape:'htmlall':'UTF-8'}" title="{l s='Delete' mod='marketplace'}" class="delete_manuf_product" style="color:#2fb5d2;cursor:pointer;">
									<i class="material-icons">&#xE872;</i>
								</a>
							</td>
						</tr>
