			...
			<tbody>
					...
					<tr>
						<td>
                            {if isset($product.active) && ($product.active == 1) && ($currentShopId == $product.id_shop)}
                                <a href="{$link->getProductLink($product.product_id)|addslashes}" target="_blank" title="{l s='View Products' mod='marketplace'}">{$product.product_name|escape:'htmlall':'UTF-8'}</a>
                            {else}
								{$product.product_name|escape:'htmlall':'UTF-8'}
                            {/if}
							{hook h="displayMpOrderProductNameAfter" product=$product}
                        </td>
						<td>{$product.price_te|escape:'htmlall':'UTF-8'}</td>
						<td>{$product.total_tax|escape:'htmlall':'UTF-8'}</td>
                        <td>{$product.price_ti|escape:'htmlall':'UTF-8'}</td>
                        <td class="text-center">{$product.commission_data|escape:'htmlall':'UTF-8'}</td>
                        <td>{$product.admin_tax|escape:'htmlall':'UTF-8'}</td>
                        <td>{$product.seller_tax|escape:'htmlall':'UTF-8'}</td>
						<td>{$product.admin_total_commission|escape:'htmlall':'UTF-8'}</td>
						<td>{$product.seller_total_amount|escape:'htmlall':'UTF-8'}</td>
					</tr>
				...
				{/foreach}
                    <tr>
						...
						<td><strong>{$admin_commission_total|escape:'htmlall':'UTF-8'}</strong></td>
						<td><strong>{$seller_total|escape:'htmlall':'UTF-8'}</strong></td>
					</tr>
			</tbody>
		</table>
	</div>
