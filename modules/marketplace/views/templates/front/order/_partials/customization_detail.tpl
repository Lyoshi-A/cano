...
{if isset($product['customization_data'])}
	<a href="#" data-toggle="modal"
		data-target="#product-customizations-modal-{$product.id_customization|escape:'htmlall':'UTF-8'}">{l s='Product customization' mod='marketplace'}</a>
	<div class="modal fade customization-modal" id="product-customizations-modal-{$product.id_customization|escape:'htmlall':'UTF-8'}" tabindex="-1"
						...
						<div class="form-horizontal">
							{if ($customization.type == Product::CUSTOMIZE_FILE)}
								<div class="form-group">
									<span
										class="col-lg-4 control-label"><strong>{if $customization['name']}{$customization['name']|escape:'htmlall':'UTF-8'}{else}{l s='Picture #' mod='marketplace'}{$customization@iteration|escape:'htmlall':'UTF-8'}{/if}</strong></span>
									<div class="col-lg-8">
										<a href="{$link->getModuleLink('marketplace','mediadownload', ['ajax' => 1, 'action' => 'customizationImage', 'img' => $customization['value'], 'name' => $order->id|intval|cat:'-file'|cat:$customization@iteration])}"
											...
											<img class="img-thumbnail"
												src="{$smarty.const._THEME_PROD_PIC_DIR_|escape:'quotes':'UTF-8'}{$customization['value']|escape:'htmlall':'UTF-8'}_small"
												alt="" />
										</a>
									</div>
								</div>
							{elseif ($customization.type == Product::CUSTOMIZE_TEXTFIELD)}
								<div class="form-group">
									...
									<div class="col-lg-8">
										<p class="form-control-static">{$customization['value']|escape:'htmlall':'UTF-8'}</p>
									</div>
								</div>
							{/if}
						</div>
