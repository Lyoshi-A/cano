...
$(document).ready(function() {
    ...
    $('#step_carrier_range').on('submit', function(e) {
        ...
        if ($.isNumeric($('#range_info_detail .range_head_right').children('input').val())) {
            ...
            $.ajax(shipping_ajax_link, {
                type: 'POST',
                data: $('#step_carrier_range').serialize() + '&fun=range_add&action=shippingImpact&ajax=1&token=' + wk_static_token,
                dataType: 'json',
                success: function(data, status, xhr) {
                    $(".loading_overlay").hide();
                    if (data == 0) {
                        $.growl.error({ title: "", message: message_impact_price_error });
                    } else {
                        $.growl.notice({ title: "", message: message_impact_price });
    ...
    $('.mp_shipping_btnr').click(function(e) {
        e.preventDefault();
        $('#mp_shp_carrier').trigger('click');
    })
    $("#mp_shp_carrier").on("change", function() {
        ...
        if (file_name !== '') {
            if ((Math.round(this.files[0].size / 1024 / 1024) * 100) / 100 > allowed_file_size) {
                ...
                $("#mp_shp_carrier_name").val(file_name);
                $(".mp_shp_carrier_name").text(file_name);
            }
        }
    });
});
...
$(document).ready(function() {
    ...
    $("#NextButtonclick").click(function(e) {
        ...
        var transit_time = $.trim($('.seller_default_lang_class').val());
        ...
        if (shipping_name == '') {
            $.growl.error({ title: "", message: shipping_name_error });
            is_error = true;
        } else if (transit_time == '') {
            if ($('#multi_lang').val() == '1') {
                $.growl.error({ title: "", message: transit_time_error + ' ' + seller_lang_name });
            } else {
                $.growl.error({ title: "", message: transit_time_error_other });
            }
            $('.seller_default_lang_class').focus();
            is_error = true;
        } else if (isNaN(speed_grade)) {
            $.growl.error({ title: "", message: speedgradeinvalid });
            is_error = true;
        } else if (speed_grade.length != 1) {
            $.growl.error({ title: "", message: speedgradevalue });
            is_error = true;
        } else if (logo) {
            var get_ext = logo.split('.'); // split file name at dot
            get_ext = get_ext.reverse(); // reverse name to check extension
            if ($.inArray(get_ext[0].toLowerCase(), exts) <= -1) {
                $.growl.error({ title: "", message: invalid_logo_file_error });
                return false;
            }
        }
        ...
        if (is_error == true) {
            ...
            if (rel == 2) {
                ...
                if (shipping_charge_error_message != "") {
                    $.growl.error({ title: "", message: shipping_charge_error_message });
                    return false;
                }
            }
            ...
            if (nextshippingstep != 1) {
                ...
                if (nextshippingstep == 3) {
                    ...
                    document.querySelector('#FinishButtonclick').addEventListener('click', function() {
                        setTimeout(() => this.disabled = true)
                    });
                } else if (typeof(mp_shipping_id) == 'undefined' || mp_shipping_id == 0) {
    ...
    $('#PreviousButtonclick').click(function(e) {
        ...
        if (preshippingstep == 1) {
            ...
            if (preshippingstep == 3) {
                ...
                if (typeof(mp_shipping_id) == 'undefined' || mp_shipping_id == 0) {
                    $('#Finishdisablebutton').show();
                    $('#FinishButtonclick').hide();
                }
            }
    ...
    $(document).on('blur', '.rangeAllforzone', function() {
        ...
        if (isNaN(rangevalue)) {
            $.growl.error({ title: "", message: invalidNumeric });
            return false;
        } else {
...
function displayAllValueOnLoadPage() {
    ...
    if (typeof(mp_shipping_id) != 'undefined' && mp_shipping_id == '') {
        ...
        if (typeof(is_free) != 'undefined' && is_free == 1) {
            ...
            $('.rangeAllforzone').prop('disabled', true).addClass('wk-form-control').css('border-color', '#999999');
            //$('.other_input_zone').prop('disabled', true).addClass('wk-form-control').css('border-color', '#999999');
            $('.other_input_zone').addClass('wk-form-control').css('border-color', '#999999');
            $('.new_range #add_new_range').addClass('disabled');
...
function findRange() {
    ...
    var is_error = false;
    if (id_zone == -1) {
        $.growl.error({ title: "", message: zone_error });
        is_error = true;
        return false;
        ...
        $.ajax(shipping_ajax_link, {
            ...
            success: function(data, status, xhr) {
                if (data != 0) {
                    ...
                    $.growl.error({ title: "", message: no_range_available_error });
                }
            },
            error: function(xhr, status, error) {}
        });
